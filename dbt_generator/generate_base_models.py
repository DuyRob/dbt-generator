...
def generate_base_model(table_name, source_name, timezone):
	print(f'Generating base model for table {table_name}')
	bash_command = f'''
		dbt run-operation  base_table_gen --args \'{{"source_name": "{source_name}", "table_name": "{table_name}", "timezone":"{timezone}"}}'
	'''
	if system() == 'Windows':
